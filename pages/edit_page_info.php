...
?>
			<form method="post">
				...
				<tr>
					...
					<td><textarea style="width: 400px; height: 100px;" name="info"><?php print $page_info[3]; ?></textarea></td>
...
?>
		<tr>
			<form class="form" method="post" action="<?php $GLOBALS['adminpage']; ?>?page=<?php print htmlentities($GLOBALS['this_page_number']) ?>">
				...
				<input type="hidden" name="update_hinweis" value="<?php print $row[0]; ?>" />
				<td><?php print $row[1]; ?></td>
				<td><textarea style="width: 600px; height: 100px;" name="hinweis"><?php print $row[2]; ?></textarea></td>
				<td><input type="submit" value="Speichern" /></td>
			</form>
		</tr>
<?php
...
?>
		<tr>
			<form class="form" method="post" action="<?php $GLOBALS['adminpage']; ?>?page=<?php print htmlentities($GLOBALS['this_page_number']) ?>">
				...
				<td><?php print $row[1]; ?></td>
				<td><textarea style="width: 600px; height: 100px;" name="text"><?php print $row[2]; ?></textarea></td>
				<td><input type="submit" value="Speichern" /></td>
			</form>
		</tr>
<?php
